 ...
 * 渲染民族数据
 */
service.prototype.nationList = function (that) {
  //init
  var self = this;
 ...
 * 渲染学历数据
 */
service.prototype.educationList = function (that) {
  //init
  var self = this;
 ...
 * 报名
 */
service.prototype.signUp = function (that) {
  //init
  var self = this;

  //construct
  let form_data = that.vm.db.form_data;
  let params = {
    system: config.system,
    version: config.version,
    sign: null,
    ...
    nation: that.vm.nation_list[form_data.nation].mz_id,
    education: that.vm.education_list[form_data.education].id,
    mobile: form_data.mobile
  };
  if (form_data.wechat) params.wechat = form_data.wechat;
  ...
  api.post(url, params).then(res => {
    if (res.status_code == 200) {
      ...
      prepage.signUpSuccess(that.vm.db.option);
